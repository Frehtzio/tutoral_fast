...
def vote(vote:schemas.Vote,db:Session = Depends(database.get_db),current_user :int = Depends(oauth2.get_current_user)):
    ...
    if vote.dir == 1:
        if found_vote:
            raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=f"user {current_user.user_id} has already vote on post {vote.post_id}")
        
        new_vote = models.Vote(post_id = vote.post_id,user_id = current_user.user_id)
        db.add(new_vote)
        db.commit()
        # return message successs
        return {"message":"success fully voted"}
    else:
        if not found_vote:
            ...
            db.commit()
            return {"message":"succes fully deleted"}
